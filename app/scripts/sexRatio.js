...
var sexLegend = sexControls.append('div').attr('class', 'sexLegend')
...
// use only data from selected country
	...
	$('.sexLegend').html(function() {
		var a = filtered[0];
		return '<p>In 2010 <strong>' + a.country + '</strong> registered ' + commas(a.births) + ' births. At the natural rate of 105 boys for every 100 girls, ' + commas(a.natural) + ' girls should have been born that year. But at its ratio of '+ a['sex-ratio'] + ' girls for every 1,000 boys, only ' + commas(a.est) + ' were born. That means ' + commas(a.diff) + ' girls are "missing" for 2010 alone. And for every 105 boys, ' +a.node.missing+ ' more girls should have been born.</p><h3>105<svg width="25px" height="20px"><path transform=scale(1.2) class="boyPath" d="'+boyPath+'" /></svg></h3><h3>' + a.node.girls + '<svg width="25px" height="20px"><path transform=scale(1.2) class="girlPath" d="'+girlPath+'" /></svg></h3><h3>' + a.node.missing + '<svg width="25px" height="20px"><path transform=scale(1.2) class="missingPath" d="'+girlPath+'" /></svg></h3><p><em>Data are from the World Bank and UN</em></p>'
		});
